...
detailed_tosu = {}
all_urls = set()
for t in t_data["weekday"] + t_data["holiday"]:
    url = "https://www.jrkyushu-timetable.jp" + t["href"]
    all_urls.add(url)

print(f"Total Tosu unique URLs to fetch: {len(all_urls)}")
for i, url in enumerate(all_urls):
    html = fetch_url(url)
    info = parse_train_detail_html(html)
    if info:
        detailed_tosu[url] = info

with open("detailed_tosu.json", "w", encoding="utf-8") as f:
    json.dump(detailed_tosu, f, ensure_ascii=False, indent=2)

print("Done fetching Tosu trains!")
